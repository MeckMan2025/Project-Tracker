...
import { Users, Shield, Clock, Play, Square, ChevronLeft } from 'lucide-react'
...
// The original scouting-setup screen (fixed roles, groups, alliance slots,
// scouting periods) — now scoped to a single scheduled date. `scheduleId`
// identifies which date's setup row to read/write. Edit gating uses hasLeadTag
// (Coaches / Mentors / Leads / Co-Founders); everyone else views read-only.
export default function ScoutingDaySetup({ scheduleId, dateTitle, dateSubtitle, onBack }) {
  ...
  // Load this date's setup row via REST
  useEffect(() => {
    setData(null)
    async function load() {
      try {
        const res = await fetch(
          `${supabaseUrl}/rest/v1/scouting_schedule?id=eq.${scheduleId}&select=*`,
          { headers: restHeaders }
        )
        if (res.ok) {
          const rows = await res.json()
          ...
          if (!loaded.allianceSlots) loaded.allianceSlots = { ...DEFAULT_DATA.allianceSlots }
          if (!loaded.groups) loaded.groups = DEFAULT_DATA.groups.map(g => ({ ...g }))
          if (!loaded.fixedRoles) loaded.fixedRoles = DEFAULT_DATA.fixedRoles.map(r => ({ ...r }))
          setData(loaded)
        } else {
    ...
    load()
  }, [supabaseUrl, supabaseKey, scheduleId])
  ...
  }, [loadSubmissions, scheduleId])

  const autoSave = useCallback((newData) => {
    if (saveTimer.current) clearTimeout(saveTimer.current)
    saveTimer.current = setTimeout(async () => {
      const body = { id: scheduleId, data: newData, updated_by: username, updated_at: new Date().toISOString() }
      try {
  ...
  }, [username, supabaseUrl, supabaseKey, scheduleId])
        ...
        <div className="px-4 py-3 ml-14 flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-gray-500 truncate">{dateSubtitle || 'Alliance assignments & scouting accountability'}</p>
            </div>
          </div>
          <NotificationBell />
        </div>
                    ...
                    return (
                      <span
                        ...
                        className={`px-3 py-1 rounded-full text-sm font-medium ${
                          hasSubmitted ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'
                        }`}
                      >
                        {scout}
                      </span>
